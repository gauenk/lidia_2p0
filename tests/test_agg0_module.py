...
class TestAgg0(unittest.TestCase):
    ...
    def run_agg0_ntire(self,name,sigma,device):
        ...
        ntire_agg0,ntire_fold = model_ntire.run_agg0(ipatches,idists,ha,wa)
        ntire_fold = ntire_fold.detach()
        ntire_agg0 = rearrange(ntire_agg0,'t (h w) 1 d -> t h w d',h=hp).detach()/30.
        ...
        nl_agg0,nl_fold = model_nl.run_agg0(ipatches,idists,iinds,64,64)
        nl_fold = nl_fold.detach()
        nl_agg0 = rearrange(nl_agg0,'t (h w) 1 d -> t h w d',h=hp).detach()/30.
        ...
        # -- agg --
        nta = rearrange(ntire_agg0[:,:,:,13::25],'t h w c -> t c h w')
        nla = rearrange(nl_agg0[:,:,:,13::25],'t h w c -> t c h w')
        save_burst(nta,"output/tests/agg0/","ntire_agg")
        save_burst(nla,"output/tests/agg0/","nl_agg")

        # -- agg delta --
        print(ntire_agg0[0,:5,:5,0])
        print(nl_agg0[0,:5,:5,0])
        error = th.mean((ntire_agg0 - nl_agg0)**2,-1)
        print("error.shape: ",error.shape)
        error = repeat(error,'t h w -> t c h w',c=3)
        print("error.shape: ",error.shape)
