from lidia.data_modules import *
...
def process_image(nl_denoiser, image_n, max_chunk, srch_img=None):
    with torch.no_grad():
        image_dn = nl_denoiser(image_n, train=False, save_memory=True, max_chunk=max_chunk,srch_img=srch_img)
...
def adapt_net(nl_denoiser, opt, total_pad, train_image_c):
    ...
    print("train_image_c.shape: ",train_image_c.shape,total_pad)
    th_img = tensor_to_ndarray_uint8(train_image_c)
    print("th_img.shape: ",th_img.shape)
    th_img = rearrange(th_img,'b h c w -> b c h w')
    print("th_img.shape: ",th_img.shape)
    train_set = ImageDataSet(block_w=block_w_pad, images=th_img,
